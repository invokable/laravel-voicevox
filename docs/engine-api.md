...
| `POST /multi_synthesis`           | ✅                  | ✅              | synthesisを複数回実行してzip化。                                                        |
...
| `POST /initialize_speaker`        | ❌                  | ❌              | コアはvvmロードで初期化済みのためAPIとして公開する必要がない。フォールバックもなし。                                 |
...
| `GET /downloadable_libraries`     | ❌                  | ❌              | ライブラリ管理はエンジン固有の機能。フォールバックもなし。対応不要。                                            |
